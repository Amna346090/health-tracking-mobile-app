...
function NotificationsNavLink({ linkClass, onClick }: { linkClass: (p: { isActive: boolean }) => string; onClick: () => void }) {
  const { unreadCount } = useNotificationsBadge();
  return (
    <NavLink to="/notifications" className={linkClass} onClick={onClick}>
      <Bell size={17} strokeWidth={2} />
      Notifications
      {unreadCount > 0 && <span className="sidebar-nav-badge">{unreadCount}</span>}
    </NavLink>
  );
}
      ...
      <div className="app-shell">
        ...
        <aside className={`sidebar${mobileNavOpen ? ' open' : ''}`}>
          ...
          <nav className="sidebar-nav">
            ...
            <NotificationsNavLink linkClass={linkClass} onClick={closeMobileNav} />
          </nav>
