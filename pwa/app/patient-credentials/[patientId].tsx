...
import { isTempId } from '../../offline/entities/patients';
...
export default function PatientCredentialsScreen() {
  ...
  const [loading, setLoading] = useState(!pidIsTemp);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (pidIsTemp) return; // this patient hasn't synced yet — nothing to fetch until it does
    let cancelled = false;
    getPatientCredentials(pid)
      .then((data) => { if (!cancelled) setCredentials(data); })
      .catch((err) => {
        if (!cancelled) setError(err instanceof ApiError ? err.message : t('credentials.loadFailed'));
      })
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.navBar}>
          ...
          <Text style={styles.navTitle}>{t('credentials.title')}</Text>
          <View style={{ width: 60 }} />
        </View>

        {name && <Text style={styles.subtitle}>{t('credentials.autoGeneratedFor', { name })}</Text>}

        {pidIsTemp && (
          <View style={styles.pendingCard}>
            <Text style={styles.pendingText}>{t('credentials.pendingSync')}</Text>
          </View>
        )}

        {!pidIsTemp && loading && <ActivityIndicator style={styles.loading} color={colors.primary} />}
        ...
        {!pidIsTemp && !loading && !error && credentials && (
          <View style={styles.card}>
            <View style={styles.row}>
              <Text style={styles.label}>{t('credentials.usernameOrEmail')}</Text>
            ...
            <View style={styles.row}>
              ...
              {credentials.password ? (
                ...
                <Text style={styles.valueMuted}>{t('credentials.notAvailable')}</Text>
              )}
            </View>
            <Text style={styles.hint}>{t('credentials.longPressHint')}</Text>
          </View>
        )}

        {!pidIsTemp && !loading && userId && (
          <TouchableOpacity style={styles.resetLink} onPress={goToResetPassword}>
            <Text style={styles.resetLinkText}>{t('resetPassword.title')}</Text>
          </TouchableOpacity>
        )}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  pendingText: { ...typography.body2, color: colors.primary },
